...
contract FastExchange is Owned, SafeMath {
    ...
    struct Transaction {
        ...
        uint receiveTime;
        uint executedTime;
        uint status; //0: pending; 1: processing; 2: success; -1: canceled 
    }
    ...
    function getLastestTransaction(address _from) public returns (Transaction) {
        require(getTransactionBookCount(_from) > 0);
        return (transactionBook[_from][getTransactionBookCount(_from).safeSub(1)]);
    }
    ...
    function createNewTransaction(uint _ethValue, uint _estimatedTokenRate) public {
        ...
        require(transBookLength == 0 
            || transactionBook[msg.sender][transBookLength.safeSub(1)].status == 2 
            || transactionBook[msg.sender][transBookLength.safeSub(1)].status == -1);
    ...
    function executeWhenReceiveEther(address _sender, uint _ethValue) internal {
